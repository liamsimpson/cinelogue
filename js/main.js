$(function() {
    ...
    $.ajax({
        type: 'GET',
        url: "https://api.themoviedb.org/3/discover/movie?api_key=1ef4d18ee3158d098e354a7f26370ca3&release_date.gte=" + strPreDate + "&release_date.lte=" + strDate + "&language=en-US&sort_by=vote_count.asce&sort_by=release_date.desc&include_adult=false&include_video=false&page=1&region=US&vote_count.gte=210",
        ...
        success: function(data) {
                    $.each(data.results, function(i, result) {       //for each movie append the following         
                        $results.append('<div id="resultWrap"><img src="http://image.tmdb.org/t/p/original/'+result.poster_path+'"><div id="Content"<ul><li><b>' + result.original_title + '</b><li><b>' + result.release_date + '</b><li><p>' + result.overview + '</p></li><li>' + result.vote_count + ' likes </li><li>IMDb: ' + result.vote_average + '/10</li></div></div><hr>'); 
                    });
